...
public class Run {
	/*
	 * 람다식이란? 익명함수(제목이 없는 함수)처럼 구동(java8버전 이후부터 사용가능)
	 * 실제로는 익명구현객체(이름없는 클래스) 생성하여 내부에 내가 작성한 코드 실행시키는 방식으로 구동
	 * 
	 * 람다식의 장점: 표현 간결, 가독성 좋다
	 * 			  내부적으로 람다식이 활용된 메소드 많다
	 * 		  단점:람다식에 이해도가 부족하면 알아보기 힘듬
	 * 
	 * [표현식]
	 * 변수 변수명 =(매개변수) -> {구현코드};
	 */
	...
	@FunctionalInterface //내부적으로 1개의 미구현 함수를 지닌 인터페이스(람다식으로 사용한 인터페이스)
